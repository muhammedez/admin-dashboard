...
export function requireAdmin(request: Request): { userId: number; role: string } | NextResponse {
...
export function getSessionUser(request: Request): { userId: number; role: string } | null {
  const auth = request.headers.get("Authorization")
